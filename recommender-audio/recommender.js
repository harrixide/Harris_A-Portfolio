...
function renderSongComparison(recommendedSong, recommendation) {
  ...
  compareButton.className = "comparison-action"
  compareButton.textContent = "Compare Songs"
  compareButton.setAttribute("aria-label", `Compare audio for ${selectedSong.song || "the selected song"} and ${recommendedSong.song || "the recommendation"}`)
  compareButton.addEventListener("click", () => openCompareDrawer(recommendedSong))
  header.append(copy, compareButton)
...
function renderAudioControl(container, song, variant) {
  if (variant === "selected") container.innerHTML = ""
  ...
  if (!song?.youtube_id) {
    ...
    button.textContent = isCompareButton ? "Compare Songs" : "Play"
